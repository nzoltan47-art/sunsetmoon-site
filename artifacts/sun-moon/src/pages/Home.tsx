...
import { Compass, Calendar as CalendarIcon, MapPin, ChevronLeft, ChevronRight } from 'lucide-react';
import { GlassButton } from '@/components/ui/glass-button';

export default function Home() {
  const { location, loading: geoLoading, error: geoError, requestLocation, updateLocation } = useGeolocation();
  const [selectedDate, setSelectedDate] = useState<Date>(new Date());
  
  const astroData = useAstroData(location?.latitude, location?.longitude, selectedDate);
      ...
      <header className="w-full p-4 sm:p-6 lg:p-8 flex flex-col md:flex-row justify-between items-center gap-4 z-10 relative">
        <div className="flex items-center space-x-3">
          <div className="w-10 h-10 rounded-xl bg-white/10 backdrop-blur-md border border-white/20 flex items-center justify-center overflow-hidden">
             <img src={`${import.meta.env.BASE_URL}images/celestial-ornament.png`} alt="Logo" className="w-6 h-6 object-contain" />
          </div>
          <h1 className="text-xl font-display font-bold text-white tracking-widest">LUMINARY</h1>
        </div>
        ...
        <div className="flex items-center bg-black/20 backdrop-blur-md rounded-2xl border border-white/10 p-1 shadow-lg">
          ...
          <div className="px-4 py-2 flex items-center justify-center min-w-[180px] cursor-pointer hover:bg-white/5 rounded-xl transition-colors" onClick={handleToday}>
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-12 flex flex-col z-10 relative">
        ...
        {!location && geoLoading && (
          <div className="flex-1 flex flex-col items-center justify-center animate-pulse">
            <Compass className="w-16 h-16 text-white/50 mb-4 animate-spin-slow" style={{ animationDuration: '4s' }} />
            <p className="text-white/70 text-lg font-display tracking-widest">Locating your position...</p>
          </div>
        )}
        ...
        {!location && !geoLoading && (
          <div className="flex-1 flex flex-col items-center justify-center max-w-md mx-auto text-center">
            ...
            <h2 className="text-3xl font-display text-white mb-4">Where are you?</h2>
            <p className="text-white/60 mb-8 leading-relaxed">
              {geoError || "To calculate accurate celestial phases and times, we need your location. Please search for a city or enable GPS above."}
            </p>
            ...
            <div className="w-full flex flex-col items-center justify-center py-8 lg:py-16">
               ...
               {isToday ? (
                 ...
                 <div className="text-center p-8">
                    ...
                    <p className="text-white/60 uppercase tracking-widest">Viewing historical/future data</p>
                 </div>
               )}
            </div>
